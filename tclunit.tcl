...
proc tclunit::do_run_tests { testScript } {
    ...
    vwait [namespace which -variable rt](finished_file)
...
proc tclunit::capture_test_output {chan} {
    ...
    # if {$rt(phase) eq "tests"} {
    # 	test_skipped "++++ test SKIPPED $line"
    # }

    if {$rt(phase) ne "end"} {
	if {[string match "*.tcl" $line]} {
...
proc tclunit::test_passed {line} {
    ...
    #  update the GUI
    set cto(capturing) 0
    {*}$cbs(passed) $cto(filename) $cto(testName) $cto(result) [get_runtime]
    set cto(result) ""
}

proc tclunit::get_runtime {} {
	variable rt
	if { $rt(started_tests) != 0 } {
		set rt(finished_tests) [clock milliseconds]
		set time_last [expr ($rt(finished_tests) - $rt(started_tests) + 0.0)/1000]
    	set rt(started_tests) [clock milliseconds]
    	set rt(finished_tests) 0
    	return $time_last
    } else {
    	puts stderr "rt(started_tests) is 0!!"
    	return 0
    }
}
#-----------------------------------------------------------
...
proc tclunit::test_failed_continue {line} {
	...
	set cto(capturing) 0
	{*}$cbs(failed) $cto(filename) $cto(testName) $cto(result) [get_runtime]
    #}
    set cto(result) ""
}
...
proc tclunit::run_tests {path} {
    ...
    } elseif {[file extension $path] eq ".tcl"} {
